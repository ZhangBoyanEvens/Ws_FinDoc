...
export function mountOnScrollViewSwitch(refs) {
  ...
  const animateOnScroll = () => {
    items.forEach((item, itemIndex) => {
      /* 03：图左入、文案右入；06：图右入、文案左入（与原 04 相同）。04 走默认视差，与 06 置换 */
      if (itemIndex === 2 || itemIndex === 5) {
        const fromLeft = itemIndex === 2
        const slideST = {
          trigger: item.DOM.el,
          start: 'top bottom',
          end: 'top 40%',
        ...
        gsap.set(item.DOM.imageWrap, {
          xPercent: fromLeft ? -100 : 100,
        })
        gsap.to(item.DOM.imageWrap, {
          xPercent: 0,
          ease: 'power2.out',
          scrollTrigger: { ...slideST },
        })
        /* 03：文案从右入；06：文案从左入 */
        gsap.set(item.DOM.caption, {
          xPercent: fromLeft ? 100 : -100,
        })
        gsap.to(item.DOM.caption, {
          xPercent: 0,
          ease: 'power2.out',
          scrollTrigger: { ...slideST },
        })
      ...
      /* 图在右：锚在图右侧；04/08 图在左：锚在左侧，横向放大时远离文案 */
